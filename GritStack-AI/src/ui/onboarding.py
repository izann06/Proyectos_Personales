...
Estilo: Dark Mode Inmersivo (#0b0f19) + Glassmorphism + React Bits Hover Glow
"""
...
def render_onboarding():
    """
    Renderiza la pantalla inicial creativa solicitando el Token de GitHub.
    """
    load_dotenv()
...
<h1 style="font-size: 3.2rem; font-weight: 900; margin: 0 0 12px 0; background: linear-gradient(135deg, #ffffff 30%, #a855f7 70%, #38bdf8 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.03em;">
Tu Talento Técnico,<br>Decodificado por IA
</h1>
<p style="font-size: 1.12rem; color: #94a3b8; max-width: 640px; margin: 0 auto; line-height: 1.6;">
Conecta tu cuenta de GitHub para transformar tus proyectos y arquitecturas en un portafolio de alto impacto, optimizado para roles de <strong>Cloud & DevOps</strong>.
    ...
    with col_centro:
        card_start = """<div class="glass-card fade-up-2" style="padding: 32px;">
<div style="display: flex; align-items: center; gap: 14px; margin-bottom: 20px;">
<div style="width: 44px; height: 44px; border-radius: 12px; background: rgba(168, 85, 247, 0.15); border: 1px solid rgba(168, 85, 247, 0.35); display: flex; align-items: center; justify-content: center; box-shadow: 0 0 15px rgba(168, 85, 247, 0.3);">
<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#e9d5ff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
🔒 Tu token solo se utiliza en memoria para consultar la API oficial de GitHub y jamás se almacena externamente.
        ...
        if boton_conectar:
            ...
            if not token_limpio:
                st.error("⚠️ Por favor, introduce un Token de GitHub válido para continuar.")
            else:
    ...
    with c1:
        c1_html = """<div class="glass-card fade-up-1" style="padding: 22px;">
...
Escaneamos arquitecturas de infraestructura, contenedores, commits y lenguajes reales evitando métricas engañosas.
    ...
    with c2:
        c2_html = """<div class="glass-card fade-up-2" style="padding: 22px;">
<div style="font-size: 1.8rem; margin-bottom: 10px;">🏆</div>
<div style="font-family: 'Outfit'; font-weight: 700; font-size: 1.05rem; color: #ffffff; margin-bottom: 6px;">
...
Selección inteligente de tus mejores proyectos (AWS, Docker, CI/CD) y generación de README listo para publicar.
    ...
    with c3:
        c3_html = """<div class="glass-card fade-up-3" style="padding: 22px;">
<div style="font-size: 1.8rem; margin-bottom: 10px;">🎯</div>
<div style="font-family: 'Outfit'; font-weight: 700; font-size: 1.05rem; color: #ffffff; margin-bottom: 6px;">
